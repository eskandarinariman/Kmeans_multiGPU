...
int     mpi_kmeans(float[VECTORS][DIMS], int, int, int, float, int*, float**, MPI_Comm);
...
int main(int argc, char **argv) {
    ...
    extern int     optind;
           ...
           int     numClusters, numObjs,numCoords, totalNumObjs;
    ...
    float (*data)[VECTORS][DIMS] = malloc(1 * VECTORS * DIMS * sizeof(float)); // 1 was files
    if (data == NULL) {
      perror("malloc");
      return 1;
    }
    ...
    mpi_kmeans(*data, numCoords, numObjs, numClusters, threshold, membership,
